...
cmd({
    ...
    try {
        ...
        let info = `🍄 *𝚂𝙾𝙽𝙶 𝙳𝙾𝚆𝙽𝙻𝙾𝙰𝙳𝙴𝚁* 🍄\n\n` +
            ...
            `1.1 *Audio Type* 🎵\n` +
            `1.2 *Document Type* 📁\n\n` +
            `${config.FOOTER || "𓆩ObedTechX𓆪"}`;

        const sentMsg = await conn.sendMessage(from, { image: { url: image }, caption: info }, { quoted: mek });
        const messageID = sentMsg.key.id;
        await conn.sendMessage(from, { react: { text: '🎶', key: sentMsg.key } });

        // Listen for user reply — use once() to avoid listener leak
        const replyHandler = async (messageUpdate) => {
            try {
                ...
                // Remove listener now that we have the reply
        ...
        // Auto-cleanup after 2 minutes if user never replies
        setTimeout(() => conn.ev.off('messages.upsert', replyHandler), 120000);
